...
class sessions
{
  ...
  {
		self::$prefix = session_name();
		self::$expire = session_cache_expire() * 60;
  ...
  public static function _read($id)
  {
    $data = apc_fetch(self::$prefix . $id);
    	...
    	$res = db::query("SELECT `data` FROM `sessions` WHERE `id` = ?", $id)->fetch();
			...
			{
				$data = $res->data;
				apc_store(self::$prefix . $id, $data, self::$expire);
			}
		}
    return $data;
  }
